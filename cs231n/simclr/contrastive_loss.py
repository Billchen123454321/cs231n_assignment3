...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for m in range(2*N):
            if m != k:
                denominator += torch.exp(sim(z_k,out[m])/tau)
                
        loss_k = -torch.log(numerator / denominator)
        denominator = 0
        for m in range(2*N):
            if m != k+N:
                denominator += torch.exp(sim(z_k_N,out[m])/tau)
...
def sim_positive_pairs(out_left, out_right):
    ...
    norm_left = torch.linalg.norm(out_left, dim=1, keepdim=True)        # (N, D)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pos_pair = sim_positive_pairs(out_left, out_right) # (N, 1)
    sim_pos_pair = torch.cat([sim_pos_pair, sim_pos_pair], dim=0) 
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = torch.exp(sim_pos_pair/tau)
    
    
    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss_list = -torch.log(numerator/denom)
    loss = torch.sum(loss_list)/(N * 2)
