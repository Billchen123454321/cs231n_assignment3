...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    image_inputs = torch.stack([
        clip_preprocess(Image.fromarray(image))   # Converts each NumPy image array into a PIL image.
        for image in images
    ]).to(device)
    # Encode images
    image_features = clip_model.encode_image(image_inputs)
    # Compute cosine similarity between every image and every class
    ...
    pred_classes = [class_texts[i.item()] for i in pred_indices]  # torch tensor -> number
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        clip_preprocess(Image.fromarray(img))
        for img in images
        ]).to(device)
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        # Compare query against all cached image embeddings
        similarities = get_similarity_no_loop(self.image_features, text_features)
        #shape: (num_images, 1)
        # Find top-k most similar images
        _, indices = similarities.squeeze(1).topk(k) # we do not care about values
...
def compute_iou(pred, gt, num_classes):   # gt: ground truth
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        self.model = torch.nn.Linear(inp_dim, num_classes).to(device)
